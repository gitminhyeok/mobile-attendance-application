...
- **주요 기능**: 카카오 로그인(승인제), 조건부 출석 체크(IP/시간), 월별 출석률 확인, 전체 순위, 관리자 대시보드.
...
- **승인 시스템 (Approval System)**:
  ...
  - 관리자가 승인(`approved`)하기 전까지 출석 및 정보 조회 기능 제한.
- **사용자 관리**: 
...
### 3.3. 관리자 기능 (Admin)
- **대시보드**: 
  - **가입 승인 대기**: 신규 가입자 목록 확인 및 승인 처리.
  - **장기 결석자 모니터링**: 
    - **경고**: 결석 14일 이상 (미통보 불참 제외).
    - **퇴출 대상**: 결석 21일 이상 또는 미통보 불참 2회 누적.
  - **병결자 관리**: 병결 상태인 회원은 카운트에서 제외하고 별도 관리.
- **전체 멤버 관리**:
  - **정보 수정**: 이름, 기수, 전화번호, 미통보 불참일, 병결 여부 수정.
  - **회원 삭제**: 회원 정보 및 연관된 모든 출석 기록 영구 삭제 (2중 확인).
  - **기수별 보기**: 기수별로 그룹화된 아코디언 리스트 제공.
  - **편의 기능**: 경고 대상자 명단 복사, 모바일 카드 뷰/수정 모달 지원.
...
- **users**
  ...
  - `unnotified_date1`, `unnotified_date2` (String): 미통보 불참 날짜
...
- **메인 페이지**: 동적 출석 버튼, 상태 메시지 (승인 대기/병결/경고 등 상태별 안내).
- **내 기록**: 캘린더 히트맵, 월별 출석률 그래프, 최장 연속 기록(날짜 포함).
- **관리자**: 
