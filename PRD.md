...
- **주요 기능**: 카카오 로그인, 조건부 출석 체크(IP/시간), 월별 출석률 확인, 전체 순위, 관리자 대시보드(수기 관리 포함).
...
- **사용자 관리**: 
  ...
  - **초기 이름 보존**: 가입 시점의 닉네임을 `initial_nickname`으로 저장하여, 추후 관리자가 이름을 수정하더라도 원본 식별 가능.
...
- **대시보드**: 
  - **장기 결석자 모니터링**: 
    - **경고**: 결석 14일 이상 또는 미통보 불참 1회.
    - **퇴출 예정**: 결석 21일 이상 또는 미통보 불참 2회 누적.
  - **병결자 관리**: 병결 상태인 회원은 카운트에서 제외하고 별도 관리.
- **전체 멤버 관리**:
  - **정보 수정**: 이름(실명), 기수(YY-MM), 전화번호, 미통보 불참일(YY-MM-DD), 병결 여부 수정.
  - **회원 삭제**: 회원 정보 및 연관된 모든 출석 기록 영구 삭제 (2중 확인).
  - **기수별 보기**: 기수별로 그룹화된 아코디언 리스트 제공 (No Batch 최상단).
  - **반응형 UI**: 모바일에서는 카드 리스트 및 콤팩트 수정 모달 제공.
...
## 4. UI/UX 디자인 가이드
- **디자인 컨셉**: Minimalist, Black & White with Red/Blue/Yellow Accents.
- **Framework**: Tailwind CSS.
- **메인 페이지**: 동적 출석 버튼, 상태 메시지.
- **내 기록**: 캘린더 히트맵, 월별 출석률 그래프.
- **관리자**: 
...
- [x] **v1.0**: 
  - **멤버 정보 확장**: 이름 수정, 전화번호, 기수, 미통보 불참(날짜), 병결 관리.
  - **수기 출석 시스템**: 관리자용 달력 및 일괄 출석 수정 기능.
  - **모바일 최적화**: 관리자 페이지 반응형 UI (카드 리스트, 수정 모달, 기수별 보기).
  - **회원 삭제**: 데이터 무결성을 고려한 영구 삭제 기능.
- [ ] **v1.1 (예정)**: 
  - WiFi IP 대역 정밀 재설정.
