...
- **위치 제한**: 
  ...
  - `.env`에 다중 IP 및 로컬(127.0.0.1) IP 지원.
- **시간 제한 (KST 기준)**:
  - **토요일**: 13:00 훈련 기준 (12:45~13:15 출석 / 15:00까지 지각)
  - **일요일**: 16:00 훈련 기준 (15:45~16:15 출석 / 18:00까지 지각)
  - **지각 정책**: 지각(`late`)은 기록/캘린더 출석률에는 포함되지만, **랭킹에서는 정시 출석(`present`)만 카운트**됨 (UI상 색상으로 구분).
- **중복 방지**: 1일 1회만 출석 가능.
...
## 4. UI/UX 디자인 가이드
- **디자인 컨셉**: Minimalist, Black & White with Red/Blue/Amber Accents.
- **Framework**: Tailwind CSS.
- **메인 페이지**: 
  - **인터랙티브 출석 버튼**: 대형 원형 버튼, 지문 아이콘, 펄스 및 호버 애니메이션 적용.
  - **출석 상태 시각화**: 
    - 정상 출석: Blue 테마 + 체크 아이콘.
    - 지각: Amber 테마 + 체크 아이콘.
  - **애니메이션 차트**: 도넛 차트가 0%에서 부드럽게 차오르는 효과 (0.6s).
  - **Footer**: 개발자 정보 포함.
...
- [x] **v1.1**: 
  - **UI 고도화**: 인터랙티브 출석 버튼, 차트 애니메이션 적용.
  - **로직 개선**: 지각 정책 반영(출석률 포함), 다중 IP 허용 설정.
